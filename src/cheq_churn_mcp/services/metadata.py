...
class MetadataService:
    ...
    def __init__(
        self, repository: CustomerRepository, *, allow_identifier_discovery: bool = False
    ) -> None:
        self._repository = repository
        self._allow_identifier_discovery = allow_identifier_discovery
    ...
    def describe_dataset(self) -> dict[str, object]:
        ...
        return {
            ...
            "identifier_policy": self._identifier_policy(),
            "limitations": [
    ...
    def _identifier_policy(self) -> str:
        if self._allow_identifier_discovery:
            return (
                "Trusted local demo: find_customer_ids can return at most 10 identifiers for "
                "an allowlisted, non-empty filter and stated purpose. This is not authenticated "
                "RBAC. customer_id remains unavailable to aggregate filters and groupings."
            )
        return (
            "customer_id is present only in the source schema. It is not an aggregate filter "
            "or grouping and identifier discovery is unsupported."
        )
